...
function Test-Mailbox {
    ...
    try {
        $mailbox = Get-EXOMailbox -Identity $MailboxTextBox.Text
    } catch { }
...
function Test-User {
    ...
    try {
        $user = Get-EXOMailbox -Identity $UserTextbox.Text
    } catch { }
...
function Get-FolderList {
  ...
  if ($mailbox) {
      (Get-EXOMailboxFolderStatistics $MailboxTextBox.Text).FolderPath | Where-Object {!($FolderExclusions -contains $_)} | ForEach-Object { [void] $SpecificFolderComboBox.Items.Add($_) }
...
function Get-MailboxFolderPermissions {
    ...
    if ($MailboxTextBox.TextLength -ne 0) {
        ...
        # Check if Mailbox exists
        if (Get-EXOMailbox -Identity $Mailbox) {             

            # Initialize Progress Bar
            $Progressbar.Value = 0
            $ProgressBar.Maximum =Get-EXOMailboxFolderStatistics -Identity $mailbox | Measure-Object | Select-Object -ExpandProperty Count
            ...
            if ($UserTextBox.TextLength -eq 0) {
                ...
                $sendonbehalf = Get-EXOMailbox -Identity $mailbox | Where-Object { $_.GrantSendOnBehalfTo -ne $null } | Select-Object -ExpandProperty GrantSendOnBehalfTo
                ...
                $sendasmailbox = Get-EXOMailbox -Identity $mailbox | Get-ADPermission | Where-Object { ($_.ExtendedRights -like '*send*') -and ($_.IsInherited -eq $false) -and ($_.user.tostring() -ne 'NT AUTHORITY\SELF') -and ($_.user.tostring() -notlike 'S-1-*') } | Select-Object -ExpandProperty User
                ...
                ForEach ($line in $sendasmailbox) {
                    ...
                    if (Get-EXOMailbox $user -erroraction SilentlyContinue) {
                        $user = $(Get-EXOMailbox $user).name
                    }
                ...
                $fullmailboxpermissions = Get-EXOMailbox -Identity $mailbox | Get-EXOMailboxPermission | Where-Object { ($_.IsInherited -eq $false) -and ($_.user.tostring() -ne 'NT AUTHORITY\SELF') -and ($_.user.tostring() -notlike 'S-1-*') } | Select-Object -ExpandProperty User
                ...
                ForEach ($f in (Get-EXOMailboxFolderStatistics $Mailbox)) {
                    $Progressbar.Increment(1)
                                
                    if ($DefaultAnonymousCheckBox.Checked -eq $true) {
                        $FolderAccessRights = Get-EXOMailboxFolderPermission -Identity "$mailbox`:$($f.FolderId)" 
                    } else { 
                        $FolderAccessRights = Get-EXOMailboxFolderPermission -Identity "$mailbox`:$($f.FolderId)" | Where-Object {$_.User.DisplayName -notmatch '^Default|^Anonym|^Standard'}
                    }
                ...
                # Check if User exists
                $usercheck = Get-EXOMailbox -Identity $User
...
function Set-MailboxFolderPermissions {
    ...
    if (Get-EXOMailbox -Identity $Mailbox) {
        ...
        $MailboxIdentity = (Get-EXOMailbox $mailbox).Identity 
        $MailboxName = (Get-EXOMailbox $mailbox).Name
                ...
                if (($AddRadioButton.checked -eq $true) -and (($CompleteMailboxRadioButton.checked -eq $true) -or ($SpecificFolderRadioButton.checked -eq $true))) { 
                    ...
                    if ($AccessRightsComboBox.SelectedIndex -ne -1) { 
                        ...
                        if (($SpecificFolderRadioButton.checked -eq $true) -and ($SpecificFolderComboBox.SelectedIndex -ne -1)) { 
                            ...
                            $ProgressBar.Maximum =Get-EXOMailboxFolderStatistics -Identity $mailbox | Where-Object { $_.FolderPath.Contains("$folder") -eq $True } | Measure-Object | Select-Object -ExpandProperty Count
                            ...
                            ForEach ($f in (Get-EXOMailboxFolderStatistics $mailbox | Where-Object { $_.FolderPath.Contains("$folder") -eq $True } ) ) {
                                $Progressbar.Increment(1)
